...
contract Punck is ERC1155, Ownable {
    ...
    function mintTen(uint256 numberOfNfts, uint256 coinType) public onlyOwner {
    ...
    function mintFisrt(uint256 numberOfNfts, uint256 coinType, uint256 lastInvite) public onlyOwner {
         _mint(msg.sender, coinType, numberOfNfts, _tokenUri[coinType], "");
        _saleCount[coinType] = _saleCount[coinType].add(numberOfNfts);
        ...
        for (uint i = 0; i < numberOfNfts; i++) {
            ...
            uint256 invitorId = random % lastInvite;
            _userTokens[msg.sender].push(tokenInfo(coinType, nftId));
            if (invitorId > 0) {
    ...
    function setTokenUri(uint256 _tokenId, string memory _uri) public onlyOwner {
     ...
     function validateName(string memory str) public view returns (bool){
        ...
        if(b.length > 21) return false; // Cannot be longer than 25 characters
